...
**Відомий залишковий ризик (прийнятий, не виправлений):** вузьке вікно гонки між SELECT і PUT усередині `remember()` — якщо конкурентний `flush()` прилітає саме в цей проміжок, застарілий результат все одно потрапляє в кеш (перевірено емпірично на живому Redis). Ключовий нюанс: висить він НЕ до TTL(5хв) — Laravel Redis-теги працюють через ZSET-реєстрацію ключів (`RedisTagSet`), тож застарілий запис гарантовано зникає на НАСТУПНОМУ будь-якому записі в тег (секунди-хвилини в активній системі), TTL — лише worst-case межа для тиші після гонки.
