...
**Гроші (price):** ТЗ не уточнює одиниці `price` — приклад `"price": 72500` дано без пояснення. **Наше трактування** (неоднозначність, вирішена самостійно, не вимога ТЗ): це вже мінімальні одиниці валюти (копійки/центи — 72500 = 725.00 EUR, реалістична ціна за 5 ночей), тому зберігаємо AS-IS, `unsignedInteger`, без арифметичної конвертації на вході чи виході. `currency` — окреме поле, конвертація валют не в скоупі.
